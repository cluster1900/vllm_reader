...
本书面向初中级程序员和软件工程类学生。默认读者会 Python，能够阅读类、函数、
循环和基本测试；使用过聊天模型会有帮助，prompt、token、异步与张量概念在章内补齐，
...
> **读图方法：** 箭头表示知识依赖，不表示程序调用。先沿 01–06 建立推理、请求与缓存
主线，再进入执行、编译和分布式；第一次读第 10 章可先掌握指标，硬件实验等相关前置
知识补齐后再做。
...
2026-09-08 完成十章正文通读及重点调用链的静态对照，修复记录见
[全书检查报告](../meta/full-book-review-2026-09-08.md)。每章“如何阅读本章”给出明确的
第一遍路线、术语入口和一道可立即检查答案的小练习。长题库是分次阅读材料，不要求
初学者第一次通读就掌握所有平台与并行分支。GPU 运行验证仍未完成，章节保持 draft。
