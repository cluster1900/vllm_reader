...
2026-09-08 再复核时发现旧测试把 req-B、req-A 的提交误写为按外部 ID 排序。
现已按内部提交序号返回结果；新增重复外部 ID、超过六 token 响应计划、最后请求
取消后仍交付结果的测试。重跑上面的命令后，第 02 章 8 个测试通过，默认演示输出不变。

教学响应由固定计划循环产生；教学输入校验和 Core 终态记录也比生产实现更简单。
这组测试不证明真实模型质量、完整后台资源回收或所有入口具有相同长度校验。
